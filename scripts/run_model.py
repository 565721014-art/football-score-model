...
def prepare_source(args: argparse.Namespace, output: Path) -> tuple[Path, str]:
    ...
    request = urllib.request.Request(
        args.source_url,
        headers={"User-Agent": "football-score-model/1.0"},
    )
...
def validate_configuration(
    ...
    team_a: str,
    team_b: str,
) -> tuple[str, str, pd.Timestamp, pd.Timestamp, dict, dict]:
    if team_a == team_b:
    ...
    latest_completed = frame.loc[
        frame[["home_score", "away_score"]].notna().all(axis=1), "date"
    ].max()
    if pd.isna(latest_completed):
        raise ValueError("Source contains no completed matches")
    day_before_target = target_date - pd.Timedelta(days=1)
    run_date = pd.Timestamp.now().normalize()
    automatic_cutoff = min(day_before_target, run_date, latest_completed)
    cutoff = pd.Timestamp(args.cutoff) if args.cutoff else automatic_cutoff
    ...
    if target_rows.empty:
        raise ValueError("Target fixture row is required to infer competition and venue context")
    target_row = target_rows.iloc[0]
    neutral = str(target_row["neutral"]).strip().lower() in {"true", "1", "yes"}
    target_metadata = {
        "tournament": str(target_row["tournament"]),
        "neutral": neutral,
        "team_a_is_home": bool(target_row["home_team"] == team_a),
        "source_home_team": str(target_row["home_team"]),
        "source_away_team": str(target_row["away_team"]),
    }
    cutoff_resolution = {
        "policy": "explicit_override" if args.cutoff else "earliest_safe_date",
        "target_day_minus_one": day_before_target.strftime("%Y-%m-%d"),
        "run_date": run_date.strftime("%Y-%m-%d"),
        "latest_completed_source_date": latest_completed.strftime("%Y-%m-%d"),
        "resolved_cutoff": cutoff.strftime("%Y-%m-%d"),
    }
    return team_a, team_b, cutoff, target_date, cutoff_resolution, target_metadata
...
def build_namespace(
    ...
    target_row = pd.Series({"tournament": target_metadata["tournament"], "date": target_date})
    target_competition_class = classify_competition(target_row)
    target_regression_type = (
        "world_cup"
        if target_competition_class in {"world_cup_group", "world_cup_knockout"}
        else target_competition_class
    )
    namespace["TARGET_COMPETITION_CLASS"] = target_competition_class
    namespace["TARGET_REGRESSION_TYPE"] = target_regression_type
    namespace["TARGET_KNOCKOUT"] = float(target_competition_class == "world_cup_knockout")
    namespace["TARGET_NEUTRAL"] = bool(target_metadata["neutral"])
    namespace["TARGET_TEAM_A_IS_HOME"] = bool(target_metadata["team_a_is_home"])
    return namespace
...
def audit(
    ...
    target_rows = dates.eq(target_date) & (
...
def main() -> None:
    ...
    team_a, team_b, cutoff, target_date, cutoff_resolution, target_metadata = validate_configuration(
        args, source_path, tentative_a, tentative_b
    )
    namespace = build_namespace(
        team_a, team_b, cutoff, target_date, target_metadata, source_path, output, args.simulations
    )
    ...
    result["run_config"] = {
        ...
        "neutral": target_metadata["neutral"],
        "source_home_team": target_metadata["source_home_team"],
        "source_away_team": target_metadata["source_away_team"],
        "tournament": target_metadata["tournament"],
        "target_competition_class": result["target_competition_class"],
    ...
    audit_result = audit(output, result, team_a, team_b, cutoff, target_date)
